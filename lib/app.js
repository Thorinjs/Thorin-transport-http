...
module.exports = function (app, logger) {
  ...
  const ERROR_NOT_FOUND = app.error('TRANSPORT.NOT_FOUND', 'The requested resource was not found', 404);
  const ERROR_UNAVAILABLE = app.error('TRANSPORT.UNAVAILABLE', 'The requested resource is temporary unavailable', 502);
  const ERROR_PARSE = app.error('TRANSPORT.INVALID_PAYLOAD', 'The request payload is not valid', 400);
  const ERROR_TOO_LARGE = app.error('TRANSPORT.INVALID_PAYLOAD', 'The request payload is too large', 413);
  const ERROR_ENCODING = app.error('TRANSPORT.INVALID_PAYLOAD', 'The requested encoding is not available', 415);
  const ERROR_ABORTED = app.error('TRANSPORT.ABORTED', 'The request was aborted', 400);
  const ERROR_DATA = app.error('TRANSPORT.DATA', 'Invalid or missing request information', 400, { field: 'type' });
  ...
  class ThorinExpress {
      ...
      this.#httpServer.on('error', (e) => {
        if (!isDone) {
          ...
          if (e.code === 'EADDRINUSE') {
            return done(app.error('TRANSPORT.PORT_IN_USE', `The port ${this.#config.port} or ip ${this.#config.ip} is already in use.`));
          }
          return done(app.error(e));
        }
      ...
      app.use(function (req, res, next) {
        try {
          ...
          if (!credentials || credentials.name !== authConfig.user || credentials.pass !== authConfig.password) {
            ...
            return next(app.error('UNAUTHORIZED', 'Authorization failed', 401));
          }
        } catch (e) {
          return next(e);
        }
    ...
    #registerStaticPaths = (app, paths) => {
      ...
      paths.forEach((sPath) => {
        ...
        if (sPath.charAt(0) === '/' || sPath.charAt(0) === '\\') {
          ...
          sPath = path.normalize(app.root + '/' + sPath);
        }
        try {
    ...
    #handleIncomingRequest = (actionType, alias, req, res, next) => {
      ...
      if (app.env !== 'production' && req._hasDebug) {
      ...
      const intentObj = new app.Intent(actionType, inputData, (wasError, data, intentObj) => {
        isFinished = true;
        let hasCors = false,
      ...
      function onRequestClose() {
        ...
        try {
          intentObj.__trigger(app.Intent.EVENT.CLOSE);
        } catch (e) {
        }
      }

      req.once('close', onRequestClose);
      app.dispatcher.triggerIntent(intentObj);
    }
    ...
    #handleRequestError = (err, req, res, next) => {
      ...
      let logErr, logQs = this.#getShortQuery(req),
        rawUrl = this.#getShortUrl(req);
