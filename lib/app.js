...
module.exports = function (thorin, logger) {
  ...
  class ThorinExpress {
    ...
    #registerCors = (app, corsConfig, corsIgnore) => {
      ...
      let opts = {
        domain: corsConfig,
        ignore: corsIgnore,
        corsAge: this.#config.corsAge
      };
      app.use(appCorsMiddleware(opts));
    };
    ...
    #registerActionPath = (verb, url, actionName) => {
      ...
      let self = this;
      const actionObj = thorin.dispatcher.getAction(actionName);
      // Register the OPTIONS handler
      if (actionObj.cors && !this.#config.cors) {
        handler['options'](url, appCorsMiddleware({
          domain: actionObj.cors.domain,
          credentials: actionObj.cors.credentials,
          corsAge: this.#config.corsAge,
          close: true
        }));
      }
    ...
    #setCors = (req, res) => {
      let dom = this.#config.cors,
        rawOrigin = req.headers['origin'] || req.headers['referer'] || null;
      if (dom === true) {
      ...
      if (!dom) return;
      res.header('Access-Control-Allow-Origin', rawOrigin || dom);
    ...
    #handleIncomingRequest = (actionType, alias, req, res, next) => {
      ...
      const intentObj = new thorin.Intent(actionType, inputData, (wasError, data, intentObj) => {
        ...
        let hasCors = false,
          ...
          corsCredentials,
          corsMethods;
        let rawOrigin = req.headers['origin'] || req.headers['referer'] || null;
        ...
        if (hasCors) {
          ...
          if (!corsDomain || corsDomain === '*') corsDomain = rawOrigin || '*';
          try {
    ...
    #handleRequestError = (err, req, res, next) => {
      ...
      if (req._hasDebug !== false) {
        ...
        if(reqErr?.code === 'DATA.INVALID' && reqErr?.data?.fields) {
          logMsg += ` [FIELDS ${JSON.stringify(reqErr.data.fields || reqErr.data.field)}]`;
        }
  ...
  /*
   * Cors handling in route.
   * @Options
   * - data.domain - if set, as string or array of strings - the domain to whitelist
   * - data.ignore - if set, the ignored hosts.
   * - data.corsAge - the cors age to use
   * - data.close - if set, close the response now.
   * - data.credeitnals - if set to false, do not send credentials.
   *
   *  corsConfig- if set as true, allow *
   *  corsConfig - if set as string or array of strings, only those domains allowed.
   *
   * */
  function appCorsMiddleware(data = {}) {
    let domains = [],
      ignoreHosts,
      isAllCors = false,
      sendEmpty = data.close || false,
      sendCredentials = data.credentials !== false,
      corsAge = data.corsAge;
    if (typeof data.domain === 'string') {
      domains = data.domain.split(' ');
    } else if (data.domain instanceof Array) {
      domains = data.domain;
    } else {
      isAllCors = true;
    }
    if (domains[0] === '*') isAllCors = true;
    if (typeof data.ignore === 'string') {
      ignoreHosts = [data.ignore];
    } else if (data.ignore instanceof Array) {
      ignoreHosts = data.ignore;
    }
    return function (req, res, next) {
      ...
      res.header('Access-Control-Allow-Methods', req.headers['access-control-request-methods'] || CORS_METHODS);
      if (sendCredentials) res.header('Access-Control-Allow-Credentials', 'true');
      res.header('Access-Control-Allow-Headers', req.headers['access-control-request-headers'] || '*');
      if (corsAge) res.header('Access-Control-Max-Age', (corsAge).toString());
      if (sendEmpty) {
        res.end();
      } else {
        next();
