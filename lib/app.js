...
module.exports = function (thorin) {
  ...
  function getCleanOrigin(origin) {
  ...
  function registerCors(app, corsConfig, corsIgnore) {
    ...
    app.use((req, res, next) => {
      ...
      res.header('Access-Control-Allow-Origin', getCleanOrigin(rawOrigin) || '*');
  ...
  function handleIncomingRequest(actionType, alias, req, res, next) {
    ...
    const intentObj = new thorin.Intent(actionType, inputData, (wasError, data, intentObj) => {
      ...
      if (hasCors) {
        ...
        try {
          res.header('Access-Control-Allow-Origin', getCleanOrigin(corsDomain));
  ...
  function setCors(req, res, verb) {
    let dom = this[config].cors,
      ref = req.headers['origin'] || req.headers['referer'];
    if (dom === true) {
      dom = ref || '*';
      ...
      if (!ref) {
        ...
        for (let i = 0, len = dom.length; i < len; i++) {
          if (dom[i].indexOf(ref) === -1) continue;
          found = true;
          dom = dom[i];
          break;
        }
    ...
    if (!dom) return;
    res.header('Access-Control-Allow-Origin', getCleanOrigin(dom));
    res.header('Access-Control-Allow-Methods', CORS_METHODS);
    res.header('Access-Control-Allow-Credentials', 'true');
    res.header('Access-Control-Allow-Headers', req.headers['access-control-request-headers'] || '*');
    res.header('Access-Control-Max-Age', '600');
  ...
  function registerDefaultAction(verb, url) {
    ...
    if (this[config].cors) {
      this[server].options('*', handleOpt.bind(this));
    }
