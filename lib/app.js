...
module.exports = function(thorin) {
  ...
  class ThorinExpressApp {
    ...
    * */
    addHandler(actionObj, verb, url) {
      ...
      url = url.replace(/\\/g,'/');
      if(this.running) {
        handleActionPath.call(this, verb, url, actionObj.name);
        return this;
      }
      this[paths].push({
        verb: verb,
    ...
    * */
    ...
    listen(done) {
      ...
      // handle CORS
      handleCors(app, this[config].cors);
      // Handle static assets
      handleStaticPaths(app, this[config].static);
      // Handle middleware
      handleMiddleware(app, this[config]);
      this[server] = app;
      app.listen(this[config].port, this[config].ip, (e) => {
        ...
        if(typeof this[config].actionPath === 'string') {
          handleDefaultAction.call(this, defaultHandlerVerb, this[defaultHandlerPath]);
        }
        for(let i=0; i < this[paths].length; i++) {
          let item = this[paths][i];
          handleActionPath.call(this, item.verb, item.url, item.name);
        }
        this[paths] = null;
  ...
  * */
  function handleMiddleware(app, config) {
    /* Parse Form data */
  ...
  function handleRequestNotFound(req, res, next) {
    return next(thorin.error('TRANSPORT.NOT_FOUND', 'The requested URL was not found: ' + req.url, 404));
  }
  ...
  function handleRequestError(err, req, res, next) {
    ...
    // TODO: make the loggin better.
    if(statusCode === 404) {
      logger('trace', '[404] ' + reqErr.message);
    } else if(statusCode < 500) {
      logger('trace', '[' + statusCode + '] ' + reqErr.code + " - " + reqErr.message);
    } else {
      logger('warn', '[' + statusCode + '] - ' + (reqData.id || req.url), reqErr);
    }
    try {
      res.json(reqData);
    } catch(e) {
  ...
  * */
  function handleCors(app, corsConfig) {
  ...
  function handleStaticPaths(app, paths) {
  ...
  * */
  ...
  function handleIncomingRequest(actionType, req, res, next) {
    ...
    if(this[disabledActions][actionType] === true) {
      return next(thorin.error('TRANSPORT.UNAVAILABLE', 'The requested URL is temporary unavailable: ' + req.url, 502));
    }
    logger('trace', '[START ' + req.uniqueId + '] ' + req.method.toUpperCase() +' ' + req.originalUrl + ' (' + actionType + ')');
  ...
  * */
  function handleIntentSuccess(req, res, next, data, intentObj) {
    ...
    logger('trace', '[END ' + req.uniqueId + '] ' + req.method.toUpperCase() +' ' + req.originalUrl + ' (' + data.id + ') ' + intentObj.took + 'ms');
    try {
      res.json(data);
    } catch(e) {
  ...
  * */
  function handleActionPath(verb, url, actionName) {
    var app = this[server];
  ...
  * */
  function handleDefaultAction(verb, url) {
