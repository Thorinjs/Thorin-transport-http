...
module.exports = function(thorin) {
  ...
  function handleRequestError(err, req, res, next) {
    ...
    if(req._hasDebug !== false) {
      ...
      if(statusCode === 404 && thorin.env !== 'production') {
    ...
    if (err.rawData instanceof Buffer) {
      ...
      try {
        ...
        try {
          reqData = this.parseIntentJson(reqData);
        } catch(e) {
        }
        res.json(reqData);
      } catch (e) {
  ...
  function handleIncomingRequest(actionType, alias, req, res, next) {
    ...
    if (typeof this[actions][actionType] === 'undefined') {
      return handleRequestError.call(this,thorin.error('TRANSPORT.NOT_FOUND', 'The requested resource was not found: ' + req.method + ' ' + req.url, 404), req, res, next);
    }
  ...
  function handleIntentSuccess(req, res, next, data, intentObj) {
    ...
    if (typeof data === 'string') {
      ...
      try {
        ...
        try {
          data = this.parseIntentJson(null, data);
        } catch(e) {}
        res.json(data);
      } catch (e) {
