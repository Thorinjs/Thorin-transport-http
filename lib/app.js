...
module.exports = function (thorin) {
  ...
  function handleIncomingRequest(actionType, alias, req, res, next) {
    ...
    const intentObj = new thorin.Intent(actionType, inputData, (wasError, data, intentObj) => {
      ...
      if (typeof intentObj.cors === 'object' && intentObj.cors) {
        hasCors = true;
        corsDomain = intentObj.cors.domain || '*';
        corsCredentials = intentObj.cors.credentials || false;
        if (intentObj.cors.methods) {
          corsMethods = intentObj.cors.methods;
        }
  ...
  function registerDefaultAction(verb, url) {
    ...
    function handleRequest(req, res, next) {
      ...
      let payload = (typeof req.body.payload === 'object' && req.body.payload ? req.body.payload : {}),
        filter = (typeof req.body.filter === 'object' && req.body.filter ? req.body.filter : {});
      req.url = req.action;
      req.body = payload;
      req.filter = filter;
      req.query = {};
